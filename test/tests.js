...
test ("Require.js", function () {
    ok (require, "Require is loaded.");
    ok (define, "Require is loaded.");
});
...
module('Require.js compatability');

asyncTest('require() loads the module', function() {
    expect(2);
    require.config({ baseUrl: ".." });
    require(["commonwealth"], function (c_amd){
        ok(c_amd, "Commonwealth is defined as an AMD module.");
        equal(new c_amd.State("foo").name, new c.State("foo").name, "Works the same whether you use AMD or not.");
        start();
    });
});